...
<body>
    ...
    <script>
        ...
        const ejercicios = {
            ...
            '01_8': { inputs: [{label:'SEGUNDOS', id:'seg', val:'3665'}], codigo:'public class Conversion {\n    public static void main(String[] args) {\n        int t=VALOR_SEG;\n        int h=t/3600, m=(t%3600)/60, s=t%60;\n        System.out.println(h+"h "+m+"m "+s+"s");\n    }\n}', calcular:function(v){let t=parseInt(v.seg),h=Math.floor(t/3600),m=Math.floor((t%3600)/60),s=t%60;return h+'h '+m+'m '+s+'s';} },
            '01_9': { inputs: [{label:'DINERO (S/)', id:'dinero', val:'1000'}], codigo:'public class Reparto {\n    public static void main(String[] args) {\n        double t=VALOR_DINERO;\n        double[] p={0.30,0.25,0.20,0.15,0.10};\n        for(int i=0;i<5;i++)\n            System.out.printf("Hijo %d: S/.%.2f%n",i+1,t*p[i]);\n    }\n}', calcular:function(v){let t=parseFloat(v.dinero),p=[0.30,0.25,0.20,0.15,0.10],r='';p.forEach((x,i)=>r+='Hijo '+(i+1)+': S/.'+(t*x).toFixed(2)+'\n');return r;} },
            '01_10': { inputs: [{label:'STAND (S/)', id:'stand', val:'500'},{label:'PUBLI (S/)', id:'publi', val:'300'},{label:'PERSONAL (S/)', id:'personal', val:'400'}], codigo:'public class GastosFeria {\n    public static void main(String[] args) {\n        double s=VALOR_STAND, p=VALOR_PUBLI, pe=VALOR_PERSONAL;\n        System.out.printf("Total: S/.%.2f%n",s+p+pe);\n    }\n}', calcular:function(v){let t=parseFloat(v.stand)+parseFloat(v.publi)+parseFloat(v.personal);return'Total: S/.'+t.toFixed(2);} },
            '02_1': { inputs: [{label:'MONTO (S/)', id:'monto', val:'100'}], codigo:'public class Descuento11 {\n    public static void main(String[] args) {\n        double m=VALOR_MONTO, d=m*0.11;\n        System.out.printf("Desc: S/.%.2f%n",d);\n        System.out.printf("Total: S/.%.2f%n",m-d);\n        System.out.println("Obsequio: 3 caramelos");\n    }\n}', calcular:function(v){let m=parseFloat(v.monto),d=m*0.11;return'Descuento: S/.'+d.toFixed(2)+'\nTotal: S/.'+(m-d).toFixed(2)+'\nObsequio: 3 caramelos';} },
            '02_2': { inputs: [{label:'MONTO (S/)', id:'monto', val:'200'}], codigo:'public class DobleDesc {\n    public static void main(String[] args) {\n        double m=VALOR_MONTO,d1=m*0.07,m1=m-d1,d2=m1*0.07;\n        System.out.printf("Total: S/.%.2f%n",m1-d2);\n    }\n}', calcular:function(v){let m=parseFloat(v.monto),d1=m*0.07,m1=m-d1,d2=m1*0.07;return'Desc1: S/.'+d1.toFixed(2)+'\nDesc2: S/.'+d2.toFixed(2)+'\nTotal: S/.'+(m1-d2).toFixed(2);} },
            '02_3': { inputs: [{label:'SUELDO (S/)', id:'sueldo', val:'1500'}], codigo:'public class PagoMensual {\n    public static void main(String[] args) {\n        double s=VALOR_SUELDO,b=s*0.20,d=s*0.10;\n        System.out.printf("Total: S/.%.2f%n",s+b-d);\n    }\n}', calcular:function(v){let s=parseFloat(v.sueldo);return'Bonif: S/.'+(s*0.20).toFixed(2)+'\nDesc: S/.'+(s*0.10).toFixed(2)+'\nTotal: S/.'+(s*1.10).toFixed(2);} },
            '02_4': { inputs: [{label:'MONTO (S/)', id:'monto', val:'500'}], codigo:'public class Donacion {\n    public static void main(String[] args) {\n        double m=VALOR_MONTO;\n        System.out.printf("Donacion: S/.%.2f%n",m*0.05);\n    }\n}', calcular:function(v){return'Donacion: S/.'+(parseFloat(v.monto)*0.05).toFixed(2);} },
            '02_5': { inputs: [{label:'VENTAS (S/)', id:'ventas', val:'5000'}], codigo:'public class Vendedores {\n    public static void main(String[] args) {\n        double v=VALOR_VENTAS,s=300+v*0.09;\n        System.out.printf("Sueldo: S/.%.2f%n",s);\n    }\n}', calcular:function(v){let ve=parseFloat(v.ventas);return'Base: S/.300\nComision: S/.'+(ve*0.09).toFixed(2)+'\nTotal: S/.'+(300+ve*0.09).toFixed(2);} },
            '02_6': { inputs: [{label:'BRUTO (S/)', id:'bruto', val:'2000'}], codigo:'public class ESSALUD {\n    public static void main(String[] args) {\n        double b=VALOR_BRUTO,e=b*0.09,a=b*0.10;\n        System.out.printf("ESSALUD: S/.%.2f%n",e);\n        System.out.printf("AFP: S/.%.2f%n",a);\n        System.out.printf("Neto: S/.%.2f%n",b-e-a);\n    }\n}', calcular:function(v){let b=parseFloat(v.bruto),e=b*0.09,a=b*0.10;return'ESSALUD: S/.'+e.toFixed(2)+'\nAFP: S/.'+a.toFixed(2)+'\nNeto: S/.'+(b-e-a).toFixed(2);} },
            '03_1': { inputs: [{label:'MONTO (S/)', id:'monto', val:'150'}], codigo:'public class Desc11V2 {\n    public static void main(String[] args) {\n        double m=VALOR_MONTO,d=m*0.11;\n        System.out.printf("Desc: S/.%.2f%n",d);\n        System.out.printf("Total: S/.%.2f%n",m-d);\n        System.out.println("Obsequio: caramelos");\n    }\n}', calcular:function(v){let m=parseFloat(v.monto),d=m*0.11;return'Desc: S/.'+d.toFixed(2)+'\nTotal: S/.'+(m-d).toFixed(2)+'\nObsequio: caramelos';} },
            '03_2': { inputs: [{label:'MONTO (S/)', id:'monto', val:'200'}], codigo:'public class Doble10 {\n    public static void main(String[] args) {\n        double m=VALOR_MONTO,d1=m*0.10,m1=m-d1,d2=m1*0.10;\n        System.out.printf("Total: S/.%.2f%n",m1-d2);\n    }\n}', calcular:function(v){let m=parseFloat(v.monto),d1=m*0.10,m1=m-d1,d2=m1*0.10;return'Desc1: S/.'+d1.toFixed(2)+'\nDesc2: S/.'+d2.toFixed(2)+'\nTotal: S/.'+(m1-d2).toFixed(2);} },
            '03_3': { inputs: [{label:'SUELDO (S/)', id:'sueldo', val:'2000'}], codigo:'public class Empleado {\n    public static void main(String[] args) {\n        double s=VALOR_SUELDO,b=s*0.20,d=s*0.10;\n        System.out.printf("Neto: S/.%.2f%n",s+b-d);\n    }\n}', calcular:function(v){let s=parseFloat(v.sueldo);return'Bonif: S/.'+(s*0.20).toFixed(2)+'\nDesc: S/.'+(s*0.10).toFixed(2)+'\nNeto: S/.'+(s*1.10).toFixed(2);} },
            '03_4': { inputs: [{label:'PASAJES (S/)', id:'pasajes', val:'50'}], codigo:'public class Transporte {\n    public static void main(String[] args) {\n        double p=VALOR_PASAJES;\n        System.out.printf("Desc: S/.%.2f%n",p*0.07);\n        System.out.println("Obsequio: chocolates");\n    }\n}', calcular:function(v){return'Desc: S/.'+(parseFloat(v.pasajes)*0.07).toFixed(2)+'\nObsequio: chocolates';} },
            '03_5': { inputs: [{label:'VENTAS (S/)', id:'ventas', val:'8000'}], codigo:'public class Vendedor {\n    public static void main(String[] args) {\n        double v=VALOR_VENTAS,s=300+v*0.09-v*0.11;\n        System.out.printf("Sueldo: S/.%.2f%n",s);\n    }\n}', calcular:function(v){let ve=parseFloat(v.ventas);return'Base: S/.300\nComision: S/.'+(ve*0.09).toFixed(2)+'\nDesc: S/.'+(ve*0.11).toFixed(2)+'\nTotal: S/.'+(300+ve*0.09-ve*0.11).toFixed(2);} },
            '03_6': { inputs: [{label:'BRUTO (S/)', id:'bruto', val:'2500'}], codigo:'public class AFP2 {\n    public static void main(String[] args) {\n        double b=VALOR_BRUTO;\n        System.out.printf("ESSALUD: S/.%.2f%n",b*0.09);\n        System.out.printf("AFP: S/.%.2f%n",b*0.10);\n        System.out.printf("Neto: S/.%.2f%n",b*0.81);\n    }\n}', calcular:function(v){let b=parseFloat(v.bruto);return'ESSALUD: S/.'+(b*0.09).toFixed(2)+'\nAFP: S/.'+(b*0.10).toFixed(2)+'\nNeto: S/.'+(b*0.81).toFixed(2);} },
            '04_1': { inputs: [{label:'TIPO (1,2,3)', id:'tipo', val:'1'},{label:'KM', id:'km', val:'150'}], codigo:'public class RentaCar {\n    public static void main(String[] args) {\n        int t=VALOR_TIPO; double km=VALOR_KM;\n        double[] p={0,50,70,100};\n        double total=p[t]+(km>100?(km-100)*0.5:0);\n        System.out.printf("Total: S/.%.2f%n",total);\n    }\n}', calcular:function(v){let t=parseInt(v.tipo),km=parseFloat(v.km),p=[0,50,70,100];return'Tipo: '+t+'\nTotal: S/.'+(p[t]+(km>100?(km-100)*0.5:0)).toFixed(2);} },
            '04_2': { inputs: [{label:'HORAS', id:'horas', val:'50'},{label:'PAGO/HORA', id:'pago', val:'20'}], codigo:'public class HorasExtras {\n    public static void main(String[] args) {\n        int h=VALOR_HORAS; double p=VALOR_PAGO;\n        double t=h<=40?h*p:40*p+(h-40)*p*2;\n        System.out.printf("Total: S/.%.2f%n",t);\n    }\n}', calcular:function(v){let h=parseInt(v.horas),p=parseFloat(v.pago);return'Total: S/.'+(h<=40?h*p:40*p+(h-40)*p*2).toFixed(2);} },
            '04_3': { inputs: [{label:'DIA (1-7)', id:'dia', val:'1'},{label:'HORAS', id:'horas', val:'3'}], codigo:'public class Estacionamiento {\n    public static void main(String[] args) {\n        int d=VALOR_DIA, h=VALOR_HORAS;\n        double t=(d<=5?5:8)*h;\n        System.out.printf("Total: S/.%.2f%n",t);\n    }\n}', calcular:function(v){let d=parseInt(v.dia),h=parseInt(v.horas),t=(d<=5?5:8);return'Tarifa: S/.'+t+'\nTotal: S/.'+(t*h).toFixed(2);} },
            '04_4': { inputs: [{label:'NOTA 1', id:'n1', val:'14'},{label:'NOTA 2', id:'n2', val:'15'},{label:'NOTA 3', id:'n3', val:'16'}], codigo:'public class Promedio {\n    public static void main(String[] args) {\n        double n1=VALOR_N1,n2=VALOR_N2,n3=VALOR_N3+2;\n        System.out.printf("Promedio: %.2f%n",(n1+n2+n3)/3);\n    }\n}', calcular:function(v){let n1=parseFloat(v.n1),n2=parseFloat(v.n2),n3=parseFloat(v.n3)+2;return'Nota 3 + bonus: '+n3+'\nPromedio: '+((n1+n2+n3)/3).toFixed(2);} },
            '05_1': { inputs: [{label:'N TERMINOS', id:'n', val:'5'}], codigo:'public class SerieAritmetica {\n    public static void main(String[] args) {\n        int n=VALOR_N,t=3,s=0;\n        for(int i=0;i<n;i++){s+=t;t+=7;}\n        System.out.println("Suma: "+s);\n    }\n}', calcular:function(v){let n=parseInt(v.n),t=3,s=0,r='';for(let i=0;i<n;i++){s+=t;r+=t+' ';t+=7;}return'Serie: '+r+'\nSuma: '+s;} },
            '05_2': { inputs: [{label:'N TERMINOS', id:'n', val:'5'}], codigo:'public class SerieFraccion {\n    public static void main(String[] args) {\n        int n=VALOR_N;\n        double num=2,den=5,s=0;\n        for(int i=0;i<n;i++){s+=num/den;num+=3;den+=4;}\n        System.out.printf("Suma: %.4f%n",s);\n    }\n}', calcular:function(v){let n=parseInt(v.n),num=2,den=5,s=0,r='';for(let i=0;i<n;i++){s+=num/den;r+=num+'/'+den+' ';num+=3;den+=4;}return'Serie: '+r+'\nSuma: '+s.toFixed(4);} },
            '05_3': { inputs: [{label:'NUMERO', id:'num', val:'2468'}], codigo:'public class Digitos {\n    public static void main(String[] args) {\n        int n=VALOR_NUM,sp=0,si=0,c=0;\n        while(n>0){int d=n%10;c++;if(d%2==0)sp+=d;else si+=d;n/=10;}\n        System.out.println("Pares: "+sp+" Impares: "+si);\n    }\n}', calcular:function(v){let n=parseInt(v.num),sp=0,si=0,c=0;while(n>0){let d=n%10;c++;d%2==0?sp+=d:si+=d;n=Math.floor(n/10);}return'Digitos: '+c+'\nSuma Pares: '+sp+'\nSuma Impares: '+si;} },
            '05_4': { inputs: [{label:'N TERMINOS', id:'n', val:'5'}], codigo:'public class Factorial {\n    public static void main(String[] args) {\n        int n=VALOR_N,i=1; double s=0,f=1;\n        do{f*=i;s+=(i%2==0)?-1.0/f:1.0/f;i++;}while(i<=n);\n        System.out.printf("Suma: %.6f%n",s);\n    }\n}', calcular:function(v){let n=parseInt(v.n),i=1,s=0,f=1;do{f*=i;s+=(i%2==0)?-1/f:1/f;i++;}while(i<=n);return'Suma: '+s.toFixed(6);} },
            '05_5': { inputs: [{label:'PRODUCTO (1-4)', id:'prod', val:'1'},{label:'CANTIDAD', id:'cant', val:'3'}], codigo:'public class Cuadernos {\n    public static void main(String[] args) {\n        int p=VALOR_PROD,c=VALOR_CANT;\n        double[] pr={0,5,8,12,15};\n        System.out.printf("Total: S/.%.2f%n",pr[p]*c);\n    }\n}', calcular:function(v){let p=parseInt(v.prod),c=parseInt(v.cant),pr=[0,5,8,12,15];return'Total: S/.'+(pr[p]*c).toFixed(2);} },
            '05_6': { inputs: [{label:'N PRODUCTOS', id:'n', val:'5'}], codigo:'public class Stock {\n    public static void main(String[] args) {\n        int n=VALOR_N;\n        for(int i=0;i<n;i++){\n            int s=(int)(Math.random()*100);\n            String r=s<30?"BAJO":s<70?"MEDIO":"ALTO";\n            System.out.println("P"+(i+1)+": "+s+" ("+r+")");\n        }\n    }\n}', calcular:function(v){let n=parseInt(v.n),r='';for(let i=0;i<n;i++){let s=Math.floor(Math.random()*100),c=s<30?'BAJO':s<70?'MEDIO':'ALTO';r+='P'+(i+1)+': '+s+' ('+c+')\n';}return r;} }
        };
        const data = ejercicios[key];
        function init() {
            if (!data) { document.getElementById('inputs-container').innerHTML = '<p style="color:#9ca3af;">Ejercicio en desarrollo</p>'; document.getElementById('codigo').textContent = '// Código próximamente...'; return; }
            let html = '';
            data.inputs.forEach(inp => { html += '<div class="input-group"><label>'+inp.label+'</label><input type="number" id="'+inp.id+'" value="'+inp.val+'" step="any"></div>'; });
        ...
        function actualizarCodigo() {
            if (!data) return;
            let cod = data.codigo;
            data.inputs.forEach(inp => { let val = document.getElementById(inp.id) ? document.getElementById(inp.id).value : inp.val; cod = cod.replace(new RegExp('VALOR_'+inp.id.toUpperCase(), 'g'), val); });
            document.getElementById('codigo').textContent = cod;
        }
        function calcular() {
            if (!data) return;
            let valores = {};
            data.inputs.forEach(inp => { valores[inp.id] = document.getElementById(inp.id).value; });
